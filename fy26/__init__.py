...
async def project(request: Request, project_id):
    result = await fetch_json(
        f"https://apps.dvrpc.org/ords/workprogram{yr}/workprogram/projects?proid={project_id}"
    )
    if not result:
...
async def amendment(request: Request, amendment_id):
    result = await fetch_json(
        f"https://apps.dvrpc.org/ords/workprogram{yr}/workprogram/draftamendment?amendmentID={amendment_id}"
    )
    if not result:
...
async def monthlyreport(request: Request, mon):
    pageno = request.args.get("page", "1")
    result = await fetch_json(
        f"https://apps.dvrpc.org/ords/workprogram{yr}/workprogram/monthlyReports?repMonth={mon}"
    )
    if not result:
