...
async def project(request: Request, project_id):
    ...
    result["items"][0]["funding_details"] = json.loads(
        result["items"][0]["funding_details"])
    result["items"][0]["lrpimages"] = json.loads(
        result["items"][0]["lrpimages"])
    csstemplate = await render("fy26/styles.css", context=result)
    resultcss = result.copy()
    resultcss["css"] = csstemplate.body.decode()
    pretemplate = await render("fy26/pre.html", context=resultcss)
    template = await render("fy26/project.html", context=result)
    posttemplate = await render("fy26/post.html", context=result)
    submission = await fetch_file("https://cloud.dvrpc.org/api/pdf_gen/pdf", method="POST", data=FormData({"html": pretemplate.body.decode() + template.body.decode() + posttemplate.body.decode(), "css": csstemplate.body.decode()}))
