...
async def project(request: Request, project_id):
    ...
    result = await fetch_json(
        f"https://apps.dvrpc.org/ords/WORKPROGRAM27/workprogram/projects?proid={project_id}"
    )
    if not result:
        return text(f"Project {project_id} database request failed")
    if result["items"] == []:
        return text(f"Project {project_id} not found")
    csstemplate = await render("fy27/styles.css", context={"pageno": 1})
...
async def amendment(request: Request, amendment_id):
    result = await fetch_json(
        f"https://apps.dvrpc.org/ords/WORKPROGRAM27/workprogram/draftamendment?amendmentID={amendment_id}"
    )
    if not result:
...
async def monthlyreport(request: Request, mon):
    pageno = request.args.get("page", "1")
    result = await fetch_json(
        f"https://apps.dvrpc.org/ords/WORKPROGRAM27/workprogram/monthlyReports?repMonth={mon}"
    )
    if not result:
