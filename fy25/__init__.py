...
async def project(request: Request, project_id):
    result = await fetch_json(
        f"https://apps.dvrpc.org/ords/workprogram25new/workprogram/projects?proid={project_id}"
    )
    if not result:
    ...
    for item in result["items"]:
        item["funding_details"] = json.loads(item["funding_details"])
        item["lrpimages"] = json.loads(item["lrpimages"]) if item["lrpimages"] else []
        item["yr"] = yr
    ...
    submission = await fetch_file(
        ...
        method="POST",
        data=FormData({"html": pretemp + template + posttemp, "css": resultcss["css"]}),
    )
...
async def amendment(request: Request, amendment_id):
    result = await fetch_json(
        f"https://apps.dvrpc.org/ords/workprogram25new/workprogram/draftamendment?amendmentID={amendment_id}"
    )
    if not result:
...
async def chapter(request: Request, chapter_id):
    ...
    result = await fetch_json(
        f"https://apps.dvrpc.org/ords/workprogram{yr}/workprogram/projects?chapterno={chapter_id[0]}&subsection={chapter_id.ljust(2).upper()[1]}&showlive=T"
    )
    if not result:
    ...
    csstemplate = await render(
        "fy26/styles.css", context={"pageno": pageno, "chapter": chapter}
    )
    resultcss = result.copy()
    resultcss["css"] = csstemplate.body.decode() if csstemplate.body is not None else ""
    pretemplate = await render("fy26/pre.html", context=resultcss)
...
async def monthlyreport(request: Request, mon):
    pageno = request.args.get("page", "1")
    result = await fetch_json(
        f"https://apps.dvrpc.org/ords/workprogram25new/workprogram/monthlyReports?repMonth={mon}"
    )
    if not result:
